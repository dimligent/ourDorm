...
countdownRouter.post("/createCountdown", authMiddleware, async (c) => {
  try {
    ...
    const maxOrderResult = (await c.env.DB.prepare(
      "SELECT MAX(orderIndex) as maxOrder FROM countdown",
    ).first()) as any;
    ...
    await c.env.DB.prepare(
      ...
      .bind(title, targetDate, orderIndex, createTime)
      .run();
...
countdownRouter.get("/getCountdowns", async (c) => {
  try {
    const countdownsResult = await c.env.DB.prepare(
      "SELECT * FROM countdown ORDER BY orderIndex",
    ).all();
    const countdowns = countdownsResult.results as any[];
...
countdownRouter.post("/updateCountdown", authMiddleware, async (c) => {
  try {
    ...
    const existing = await c.env.DB.prepare(
      "SELECT * FROM countdown WHERE countdownId = ?",
    )
      .bind(resolvedId)
      .first();
    ...
    await c.env.DB.prepare(
      ...
      .bind(title, targetDate, resolvedId)
      .run();
...
countdownRouter.post("/deleteCountdown", authMiddleware, async (c) => {
  try {
    ...
    await c.env.DB.prepare("DELETE FROM countdown WHERE countdownId = ?")
      .bind(resolvedId)
      .run();
